...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        if(kp.pt.x > 8 && kp.pt.y > 8 && image.cols-kp.pt.x > 8 && image.rows-kp.pt.y >8){
            int m_01=0,m_10=0;
            ...
            for(int v=1;v < half_patch_size;v++){
                int v_sum = 0;
                for(int u = -half_patch_size;u < half_patch_size;u++){
                    m_10 += u * (center[u + v*step] + center[u-v*step]);
                    v_sum += center[u + v*step] - center[u - v*step];
                }
                m_01 += v_sum;
            }
    ...
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            if(kp.pt.x + u_p < 0 || kp.pt.x+u_p > image.cols || kp.pt.y+v_p < 0 || kp.pt.y+v_p > image.rows
                ...
                float u_qr=a*u_q - b*v_q, v_qr = b*u_q + a*v_q;//rotate the u ,v
                if(image.at<uchar>(cvRound(kp.pt.y + v_pr),cvRound(kp.pt.x + u_pr)) > image.at<uchar>(cvRound(kp.pt.y+v_qr),cvRound(kp.pt.x+u_qr))){
                    d[i] = 0;
                } else{
                    d[i] = 1;
                }
