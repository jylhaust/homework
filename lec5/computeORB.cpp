	...
	// START YOUR CODE HERE (~7 lines)
        ...
        for(int u = -half_patch_size;u<half_patch_size;u++)
            for(int v = -half_patch_size;v<half_patch_size;v++)
            {
                ...
                {
                    m01+=(v)*image.at<uchar>(cvRound(kp.pt.y)+v,cvRound(kp.pt.x)+u);
                    m10+=(u)*image.at<uchar>(cvRound(kp.pt.y)+v,cvRound(kp.pt.x)+u);
                }  
            }
        kp.angle = atan2((float)m01, (float)m10)*180/pi;
        // kp.angle = atan((float)m01/m10)*180/pi;
        // cout<<kp.angle<<endl;
        // cout<<kp.pt.x<<endl;
        // cout<<image.at<int>(kp.pt.y,kp.pt.x)<<endl;
        // cout<<image.at<uchar>(cvRound(kp.pt.x),cvRound(kp.pt.y))<<endl;
        // cout<<image.at(kp.pt.x,kp.pt.y)<<endl;
        // END YOUR CODE HERE
    }
    return;
}
